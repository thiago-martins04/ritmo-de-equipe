  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`relative bg-white w-full max-w-md transform transition-all duration-300 shadow-2xl rounded-sm overflow-hidden border-l-8 border-lime-400 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <button 
          onClick={handleClose}
          className="absolute top-4 right-4 text-white z-20 bg-zinc-900/50 hover:bg-zinc-900 p-1 rounded-full transition-all shadow-lg"
        ...
        <div className="relative h-64 overflow-hidden bg-zinc-100">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="bg-lime-400 text-zinc-900 text-[10px] font-bold px-3 py-1 uppercase tracking-[0.2em]">Lançamento</span>
          </div>
        </div>
        ...
        <div className="p-8">
          ...
          <div className="space-y-3">
            <a
              ...
              className="flex items-center justify-center w-full bg-zinc-900 text-white font-bold uppercase tracking-widest py-4 hover:bg-zinc-800 transition-all group shadow-lg"
            >
              Eu quero a minha
              <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform text-lime-400" />
            </a>
            
            <button 
              onClick={handleClose}
              className="w-full text-center text-zinc-400 text-xs font-bold uppercase tracking-widest hover:text-zinc-600 transition-colors py-2"
            >
              Agora não, obrigado
            </button>
          </div>
